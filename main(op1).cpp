...
int main()
{
    ...
    int vie,valj,valo,nbmin,commence,recommence;// valj=valeur du joueur , valo=valeur de l'ordinateur
    ...
    do
    {
		...
		while ( nbmin<=0 )
		{
			...
			cout <<"( La valeur doit être strictement positive. )"<<endl;
			cin >> nbmin;
		}
		...
		cout <<"Comptez de 1 en 1 égal ou supérieur à "<<nbmin<<" ."<<endl;
		...
		cout <<"Tapez STOP quand vous voulez arrêter le jeu ."<<endl;
		...
		do
		{
			...
			while ( valjs!=valjse )
			{
				...
				cout <<nom<<"("<<vie<<" vies)"<<" > ";
				cin >>valjs;
				//vérifier si le joueur veut arrêter le jeu
				if ( valjs=="STOP" )
                {
                    break;
                }
			...
			mult5 = (valo%5==0) ;
			//si 'valo' est le multiple de 3 ou 5 , on change 'valos'(le message affiché) à Fizz/Buzz/FizzBuzz
			//utilisation de  la concaténation pour générer le mot FizzBuzz
			if ( mult3 )
			{
				valos="Fizz";
			}
